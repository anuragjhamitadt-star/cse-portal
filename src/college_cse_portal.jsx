...
export default function App() {
  ...
  const seededRef=useRef({}); // tracks which collections have already been seeded
  ...
  useEffect(()=>{
    const unsubs=[];
    let ready=0;
    const markReady=()=>{ready++;if(ready>=11){dbReadyRef.current=true;setDbReady(true);}};

    // resources (single doc)
    unsubs.push(onSnapshot(doc(db,"portal","resources"),snap=>{
      if(snap.exists()){
        setResourcesLocal(snap.data());
      } else if(!seededRef.current["resources"]){
        seededRef.current["resources"]=true;
    ...
    unsubs.push(onSnapshot(doc(db,"portal","creds"),snap=>{
      if(snap.exists()){
        setCredsLocal(snap.data());
      } else if(!seededRef.current["creds"]){
        seededRef.current["creds"]=true;
    ...
    unsubs.push(onSnapshot(collection(db,"faculty"),snap=>{
      if(snap.empty){
        if(!seededRef.current["faculty"]){
          seededRef.current["faculty"]=true;
          const batch=writeBatch(db);
  ...
  const setCrewMembers=useCallback(updater=>{
    setCrewMembersLocal(prev=>{
      ...
      next.forEach(m=>batch.set(doc(db,"crewMembers",m.id),m));
      batch.commit().catch(console.error);
      return next;
    });
  },[]);
  ...
  useEffect(()=>{
    ...
    if(adminRole==="owner"){
      ...
      const member=creds.members.find(m=>m.id===adminRole);
