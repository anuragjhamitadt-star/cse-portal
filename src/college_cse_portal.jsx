...
export default function App() {
  ...
  // tracks which collections have already been seeded — MUST persist across page reloads
  // useRef({}) would reset every reload, causing re-seeding and overwriting real data
  const seededRef=useRef((()=>{try{const s=localStorage.getItem("cse_seeded_v1");return s?JSON.parse(s):{};}catch{return{};}})());
  ...
  useEffect(()=>{
    ...
    // helper: mark a key as seeded and persist it to localStorage so it survives page reloads
    const markSeeded=(key)=>{
      seededRef.current[key]=true;
      try{localStorage.setItem("cse_seeded_v1",JSON.stringify(seededRef.current));}catch{}
    };
    ...
    unsubs.push(onSnapshot(doc(db,"portal","resources"),snap=>{
      ...
      } else if(!seededRef.current["resources"]){
        markSeeded("resources");
        setDoc(doc(db,"portal","resources"),INITIAL_RESOURCES).catch(()=>{});
      }
      markReady("resources");
    ...
    unsubs.push(onSnapshot(doc(db,"portal","creds"),snap=>{
      if(snap.exists()){
        ...
        setCredsLocal(data);
      } else if(!seededRef.current["creds"]){
        markSeeded("creds");
        setDoc(doc(db,"portal","creds"),DEFAULT_CREDS).catch(()=>{});
      }
      markReady("creds");
    ...
    unsubs.push(onSnapshot(collection(db,"posts"),snap=>{
      if(snap.empty){
        if(!seededRef.current["posts"]){
          // Only seed once on very first ever load — after that, empty = admin deleted everything
          markSeeded("posts");
          const batch=writeBatch(db);
