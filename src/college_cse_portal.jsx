...
function NotificationPermissionDialog({onClose}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"rgba(0,0,0,0.75)",animation:"fadeIn 0.35s ease"}}>
      <div style={{background:"var(--card)",border:"2px solid var(--accent)",borderRadius:22,padding:"36px 32px 28px",maxWidth:440,width:"100%",boxShadow:"0 32px 80px rgba(0,0,0,0.6), 0 0 60px rgba(99,102,241,0.2)",animation:"fadeIn 0.35s ease",position:"relative",textAlign:"center"}}>
        {/* Glow ring */}
        <div style={{position:"absolute",inset:-2,borderRadius:24,background:"linear-gradient(135deg,rgba(99,102,241,0.4),rgba(167,139,250,0.4))",zIndex:-1,filter:"blur(8px)"}}/>
        {state==="granted"?(
          <>
          ...
          <>
            <div style={{fontSize:"3.5rem",marginBottom:16,filter:"drop-shadow(0 0 20px rgba(99,102,241,0.6))"}}>🔔</div>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:"1.4rem",marginBottom:8,background:"linear-gradient(135deg,#e8eaf0,#a78bfa)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>Stay Updated!</div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",marginBottom:28}}>
              {["📢 New Notices","📅 Event Alerts","🏆 Announcements","📝 Deadlines"].map(t=>(
                <span key={t} style={{background:"rgba(99,102,241,0.15)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:20,padding:"5px 14px",fontSize:"0.78rem",fontWeight:600}}>{t}</span>
              ))}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <button onClick={handleEnable} disabled={state==="asking"} style={{...BS("var(--accent)"),padding:"14px 24px",fontSize:"1rem",fontWeight:800,borderRadius:14,boxShadow:"0 0 30px rgba(99,102,241,0.45)",fontFamily:"var(--font-head)",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
...
function ClubEvents({adminLoggedIn,posts,setPosts,ticketEvents,onOpenTicket,focusPostId}) {
  ...
  const renderPost=(p)=>(
    <div key={p.id}
      ...
      style={{background:"var(--card)",border:p.pinned?"2px solid var(--accent)":"1.5px solid var(--border)",borderRadius:14,padding:"18px 20px",animation:"fadeIn 0.3s ease",position:"relative",boxShadow:focusPostId===p.id?"0 0 0 3px rgba(99,102,241,0.4)":newPostId===p.id?"0 0 20px rgba(99,102,241,0.5)":"none"}}>
        ...
        <div style={{marginBottom:showPostForm?0:16}}>
          {!showPostForm?(
            <button
              ...
              style={{...BS("var(--accent)"),padding:"11px 22px",fontSize:"0.92rem",fontWeight:800,fontFamily:"var(--font-head)",display:"flex",alignItems:"center",gap:8,borderRadius:12,boxShadow:"0 0 24px rgba(99,102,241,0.3)",transition:"all 0.2s",width:"100%",justifyContent:"center",marginBottom:16}}
              onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-1px)";e.currentTarget.style.boxShadow="0 4px 32px rgba(99,102,241,0.5)";}}
...
const SOCIAL_PLATFORMS = [
  ...
  { key:"email",      label:"Other Email", icon:"📧", color:"#6366f1", placeholder:"mailto:you@example.com" },
  ...
  return(
    <div style={{maxWidth:680,margin:"0 auto",padding:"0 0 40px"}}>
      <div style={{textAlign:"center",marginBottom:36,padding:"32px 24px 0"}}>
        <div style={{display:"inline-block",background:"rgba(99,102,241,0.12)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:100,padding:"5px 16px",fontSize:"0.75rem",fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--accent2)",marginBottom:16}}>✦ Community</div>
...
function AdminJoinRequests({requests,setRequests}){
  ...
  const STATUS_COLORS={pending:"#f59e0b",done:"#10b981",hired:"#6366f1",rejected:"#ef4444",timepass:"#94a3b8"};
          ...
          <div key={r.id} style={{background:"var(--card)",border:"1.5px solid var(--border)",borderRadius:14,marginBottom:12,borderLeft:`4px solid ${statusColor}`,animation:"fadeIn 0.2s ease"}}>
            ...
            {isExpanded&&(
              <div style={{padding:"0 18px 18px",borderTop:"1px solid var(--border)"}}>
                ...
                <div style={{background:"rgba(99,102,241,0.06)",borderRadius:10,padding:"12px 16px",marginBottom:14}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>setStatus(r.id,"hired")} style={{...BS(r.status==="hired"?"#6366f1":"transparent"),color:r.status==="hired"?"#fff":"#6366f1",border:"1.5px solid #6366f133",padding:"7px 14px",fontSize:"0.8rem"}}>🎉 Hire</button>
                ...
                {r.status==="hired"&&(
                  <div style={{marginTop:12,background:"rgba(99,102,241,0.1)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:10,padding:"10px 14px",fontSize:"0.82rem"}}>
    ...
    <>
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        @keyframes neonBlink{
          0%,100%{box-shadow:0 0 8px 2px rgba(99,102,241,0.9),0 0 24px 6px rgba(99,102,241,0.5),inset 0 0 12px rgba(99,102,241,0.15);border-color:#6366f1;}
        ...
        .neon-new-post{animation:neonBlink 0.6s ease-in-out 11 !important;border:2px solid #6366f1 !important;}
