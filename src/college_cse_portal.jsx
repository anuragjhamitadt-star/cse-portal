  ...
  return (
    <div>
      ...
      <div style={{background:"var(--card)",border:"1.5px solid var(--border)",borderRadius:14,overflow:"hidden"}}>
        ...
        {[{sem:"sem1",label:"Semester I",mk:"examMid1",ek:"examEnd1"},{sem:"sem2",label:"Semester II",mk:"examMid2",ek:"examEnd2"}].map(({sem,label,mk,ek})=>(
...
function PostForm({initial,onSave,onCancel,label,ticketEvents}) {
  ...
  const submit=()=>{
    if(!f.title.trim()||!f.body.trim()){alert("Fill title and body.");return;}
  ...
  return (
    <div style={{background:"var(--card)",border:"2px dashed var(--accent)",borderRadius:14,padding:"20px",marginBottom:24}}>
      ...
      <textarea placeholder="Body / Description" value={f.body} onChange={e=>setF(p=>({...p,body:e.target.value}))} rows={4} style={{...IS,marginBottom:10,resize:"vertical"}}/>
          ...
          <div key={p.id} ref={focusPostId===p.id?focusRef:null}
            ...
            {editId===p.id?(
              ...
              <>
                ...
                <div style={{fontSize:"0.88rem",lineHeight:1.7,opacity:0.8,marginBottom:12,whiteSpace:"pre-wrap"}}>{p.body}</div>
                {p.poll&&(
                  <div style={{background:"rgba(99,102,241,0.08)",borderRadius:12,padding:"14px",marginBottom:12}}>
...
//  ADMIN RESOURCES
// ════════════════════════════════════════════════════════
const SECTION_KEYS=[{key:"syllabus",label:"Syllabus",icon:"📘"},{key:"calendar",label:"Academic Calendar",icon:"📅"},{key:"pyq",label:"Previous Year Questions",icon:"📝"},{key:"notes",label:"Notes",icon:"📚"},{key:"timetable",label:"Lectures Time Table",icon:"⏱️"},{key:"examMid1",label:"Exam – Sem I Mid Term",icon:"📋"},{key:"examEnd1",label:"Exam – Sem I End Term",icon:"📋"},{key:"examMid2",label:"Exam – Sem II Mid Term",icon:"📋"},{key:"examEnd2",label:"Exam – Sem II End Term",icon:"📋"}];
