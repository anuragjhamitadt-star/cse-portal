...
function JoinUsForm({onSubmit}){
  ...
  return(
    <div id="contact" style={{maxWidth:680,margin:"0 auto",padding:"0 0 40px"}}>
...
export default function App() {
  ...
  // ── DEEP LINK: SCROLL TO #contact ON LOAD ────────────
  useEffect(()=>{
  ...
  const handleLogin=(role)=>{
    ...
    if(role==="owner"||!creds.members.find(m=>m.id===role)?.mustChangePassword){setView("admin");}
  };
  const logout=()=>{
    setAdminRole(null);
    try{localStorage.removeItem("cse_admin_role");}catch{}
    ...
    <>
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,zIndex:200,background:theme==="dark"?"rgba(10,10,20,0.93)":"rgba(240,248,244,0.93)",backdropFilter:"blur(12px)",borderBottom:"1px solid var(--border)",padding:"10px 24px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>{setView("portal");setActiveYear(null);setActiveFeature(null);}}>
      ...
      {/* ── HAMBURGER MENU DRAWER ── */}
      <HamburgerMenu isOpen={menuOpen} onClose={()=>setMenuOpen(false)} onNavigate={section=>{setView(section);setActiveYear(null);setActiveFeature(null);}}/>
